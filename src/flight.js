...
export class FlightJourney{
 ...
 reset(){this.position=new THREE.Vector3();this.normal=this.position.clone().sub(this.center).normalize();this.forward=new THREE.Vector3(0,0,-1);this.up=new THREE.Vector3(0,1,0);this.pitch=0;this.boost=0;this.speed=0;this.distance=0;this.contact=false;this.assisted=false;this.terrainFollowing=false;this.followHeight=1.5;this.inside=false;this.entryProgress=0;this.turnRate=0;}
 ...
 step(dt,boosting=false,turn=0,climb=0,thrust=0){
  ...
  if(this.terrainFollowing){
   ...
   if(thrust>0)this.followHeight=Math.max(1.2,this.followHeight+climb*.9*dt);
   ...
   if(travel>0)this.position.copy(this.center).addScaledVector(nextNormal,this.radius+terrainHeight(nextNormal)+this.followHeight);
   ...
   if(this.shellAltitude>16&&climb>0){this.terrainFollowing=false;this.forward.addScaledVector(nextNormal,.7).normalize();}
  }else{
