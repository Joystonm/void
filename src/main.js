...
const starsG=new THREE.BufferGeometry(),starPos=[],starCol=[];for(let i=0;i<580;i++){starPos.push((rand()-.5)*1500,(rand()-.5)*850,-450-rand()*650);const c=new THREE.Color().setHSL(.48+rand()*.25,.25,.25+rand()*.45);starCol.push(c.r,c.g,c.b);}starsG.setAttribute('position',new THREE.Float32BufferAttribute(starPos,3));starsG.setAttribute('color',new THREE.Float32BufferAttribute(starCol,3));scene.add(new THREE.Points(starsG,new THREE.PointsMaterial({size:.65,vertexColors:true,sizeAttenuation:true,transparent:true,opacity:.75})));
...
let rightFire=false,aimClock=0;
const keys=new Set();let paused=false,hud=true,elapsed=0,boost=0;
...
const defeated=document.createElement('div');defeated.className='defeated';defeated.hidden=true;defeated.innerHTML='<strong>VESSEL DISABLED</strong><p>Press R to restart the expedition</p>';$('flight').append(defeated);
const canvas=$('space');let dragPointer=null,lastPointerX=0,lastPointerY=0;
...
function resetView(){finishDrag();Object.assign(orbit,{active:false,yaw:0,pitch:.18,zoom:1});}
canvas.addEventListener('contextmenu',e=>e.preventDefault());
canvas.addEventListener('pointerdown',e=>{if(e.button===2){rightFire=true;canvas.setPointerCapture(e.pointerId);e.preventDefault();}});
...
canvas.addEventListener('pointerdown',e=>{
  ...
  activateOrbit();dragPointer=e.pointerId;lastPointerX=e.clientX;lastPointerY=e.clientY;
...
function reset(){
  rightFire=false;weapons.clearEffects();combat.reset();markers.replaceChildren();resetView();journey.reset();paused=false;elapsed=0;boost=0;keys.clear();
...
function toggleMenu(){if($('menu').open)$('menu').close();else $('menu').showModal();}
$('menuButton').onclick=toggleMenu;$('resume').onclick=()=>$('menu').close();$('reset').onclick=()=>{reset();$('menu').close();};function toggleHud(){hud=!hud;$('hud').classList.toggle('hidden',!hud);}$('photo').onclick=toggleHud;
addEventListener('keydown',e=>{if(['Space','ArrowUp','ArrowDown','ArrowLeft','ArrowRight','ControlLeft','ControlRight'].includes(e.code))e.preventDefault();keys.add(e.code);if(e.repeat)return;if(e.code==='KeyH')toggleHud();if(e.code==='Space')paused=!paused;if(e.code==='KeyR')reset();if(e.code==='KeyV')resetView();if(e.code==='Escape'){e.preventDefault();toggleMenu();}if(e.code==='KeyF'){if(document.fullscreenElement)document.exitFullscreen();else document.documentElement.requestFullscreen().catch(()=>{});}});addEventListener('keyup',e=>keys.delete(e.code));addEventListener('blur',()=>keys.clear());
...
function frame(){
  ...
  const stopped=paused||$('menu').open||document.hidden||combat.destroyed;
  if(!stopped&&loaded){
    elapsed+=dt;
    ...
    flightRig.quaternion.slerp(targetRotation,1-Math.exp(-5*dt));
    ...
    ship.rotation.z=THREE.MathUtils.damp(ship.rotation.z,-dx*.28,4,dt);
    ship.rotation.x=THREE.MathUtils.damp(ship.rotation.x,dy*.1+Math.sin(elapsed*.5)*.012,4,dt);
  ...
  combatHud.textContent=combat.loadingError?'PATROL ASSETS UNAVAILABLE':!combat.templates?'PATROL SCAN...':`SHIELD ${Math.ceil(combat.shield)} / HULL ${Math.ceil(combat.hull)} / WAVE ${combat.wave} / HOSTILES ${combat.engagedCount} / APPROACHING ${combat.inboundCount} / NEXT ${combat.secondsToWave}s${combat.incomingCount?' / INCOMING '+combat.incomingCount:''} / KILLS ${combat.kills}`;
  ...
  for(const enemy of combat.enemies){
    ...
    if(!label.hidden){label.style.left=(point.x*.5+.5)*100+'%';label.style.top=(-point.y*.5+.5)*100+'%';label.style.opacity=String(enemy.opacity);label.textContent=enemy.state==='INBOUND'?'INBOUND / '+(enemy.root.position.distanceTo(worldShip)*.1).toFixed(1)+' km':enemy.name+' '+Math.ceil(enemy.hp/enemy.maxHP*100)+'%';}
  }
  ...
  weaponHud.textContent='TWIN PULSE / CTRL OR RIGHT CLICK / '+(weapons.projectiles.length?'FIRING':weapons.enemyTarget?'ENEMY LOCK':weapons.aimPoint?'AIM ON TERRAIN':'READY');
