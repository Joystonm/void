...
export class AtmosphereCombat{
 ...
 reset(){
  ...
  this.enemies=[];this.bolts=[];this.effects=[];this.shield=100;this.hull=100;this.kills=0;this.time=0;this.lastDamage=-10;this.spawned=false;this.active=false;this.wave=0;this.nextWaveAt=0;this.pending=[];this.nextEnemyId=0;this.maxActive=4;this.patrolStarted=false;this.patrolComplete=false;this.waveCleared=false;this.nextShotAt=0;
 }
 ...
 spawn(journey){
  ...
  if(this.wave>0&&this.aliveCount===0&&this.pending.length===0&&!this.waveCleared){
   ...
   if(this.wave===3){this.patrolComplete=true;return;}
   this.nextWaveAt=this.time+15;
  }
  ...
  while(this.pending.length&&this.aliveCount<this.maxActive){
   ...
   const heavy=false,frigate=tier===3&&slot===0;
   const size=heavy?3.2:frigate?2.5:2;
   const hp=Math.round((heavy?220:frigate?132:88)*(1+(tier-1)*.1));
   ...
   const id=this.nextEnemyId++,phase=(slot-(batch.total-1)/2)*.28,range=12+(id%4)*2;
   ...
   this.scene.add(root);this.enemies.push({root,engine,trail,fadeMaterials,fadeRestored:false,opacity:0,arrivalAge:0,state:'INBOUND',flightSpeed:12,speedLimit:12,spawnDistance:root.position.distanceTo(journey.position),id,tier,phase,range,patrolForward:forward.clone(),name:(heavy?'DREADNOUGHT':frigate?'WARDEN FRIGATE':'RAIDER')+' / T'+tier,hp,maxHP:hp,radius:size*.7,velocity:new THREE.Vector3(),fireIn:4+(slot%7)*.5,fireInterval:4.5-tier*.35,damage:frigate?8:6,boltSpeed:7+tier,moveSpeed:1.1+tier*.08,dead:false});
   batch.remaining--;batch.nextLaunchAt=this.time+2.5;if(batch.remaining===0)this.pending.shift();
  }
 ...
 damageEnemy(enemy,damage,point){if(enemy.dead)return;enemy.hp=Math.max(0,enemy.hp-damage);this.burst(point,.15);if(enemy.hp===0){enemy.dead=true;enemy.root.visible=false;this.kills++;this.burst(enemy.root.position,enemy.radius);}}
 damagePlayer(damage){if(this.time-this.lastDamage<.65)return;const absorbed=Math.min(this.shield,damage);this.shield-=absorbed;this.hull=Math.max(0,this.hull-(damage-absorbed));this.lastDamage=this.time;}
 shoot(enemy,playerPosition,journey){
  const from=enemy.root.position.clone();
  const aim=playerPosition.clone();
 ...
 update(dt,journey,playerPosition,playerRadius,groundRay){
  ...
  for(const enemy of this.enemies){if(enemy.dead)continue;enemy.root.visible=true;
   ...
   // Keep the patrol bearing independent of the player's aim.
   const patrolForward=enemy.patrolForward.clone().projectOnPlane(journey.normal).normalize();
   const patrolRight=new THREE.Vector3().crossVectors(patrolForward,journey.normal).normalize();
   const angle=enemy.phase+Math.sin(this.time*.16+enemy.id*1.8)*.1,range=enemy.range;
   const desired=journey.position.clone().addScaledVector(patrolForward,Math.cos(angle)*range).addScaledVector(patrolRight,Math.sin(angle)*range);
   ...
   if(enemy.state==='INBOUND'&&remaining<5&&enemy.root.position.distanceTo(playerPosition)<35){enemy.state='ENGAGING';enemy.fireIn=3+(enemy.id%3)*.7;}
   ...
   enemy.fireIn-=dt;enemy.engine.material.color.setHex(enemy.fireIn<1?0xffee88:0xff5e38);enemy.engine.scale.setScalar(enemy.fireIn<1?2.4:1);
   if(enemy.fireIn<=0){const direction=playerPosition.clone().sub(enemy.root.position).normalize(),distance=enemy.root.position.distanceTo(playerPosition);if(distance<45&&this.time>=this.nextShotAt&&!groundRay(enemy.root.position,direction,distance)){this.shoot(enemy,playerPosition,journey);this.nextShotAt=this.time+1.1;enemy.fireIn=enemy.fireInterval;}}
  }
  for(let i=this.bolts.length-1;i>=0;i--){const bolt=this.bolts[i],travel=bolt.speed*dt,ground=groundRay(bolt.position,bolt.direction,travel),groundDistance=ground?ground.distanceTo(bolt.position):Infinity;
   ...
   if(t>=0&&t<=travel&&t<groundDistance){if(journey.dodgeTime<=0)this.damagePlayer(bolt.damage);bolt.life=0;}else if(ground)bolt.life=0;else{bolt.position.addScaledVector(bolt.direction,travel);bolt.mesh.position.copy(bolt.position);}
